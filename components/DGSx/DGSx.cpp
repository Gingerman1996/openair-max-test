...
void DGSx::setContinuousMode(bool enable, uint16_t intervalSeconds) {
  ...
  if (enable) {
    ESP_LOGI(TAG, "Continuous mode enabled with %d second interval", intervalSeconds);
    requestMeasurement(); // Start with first measurement
  } else {
    ESP_LOGI(TAG, "Continuous mode disabled");
  }
}

const char* DGSx::getLastError() {
  return _lastError;
}
...
bool DGSx::_parseDataLine(const char* line, Data &data) {
  ...
  if (strstr(line, "LPDACDAT") != nullptr && strstr(line, "LPTIACON") != nullptr) {
  ...
  if (strstr(line, "Serial, PPB, Tx100") != nullptr || strstr(line, "PPB") != nullptr) {
    ...
    if (comma2 != nullptr) {
      ...
      if (ppb_len < sizeof(ppb_str)) {
        ...
        if (ppb_value != 0.0 || *ppb_start == '0') {
          ...
          // Use detected gas type from barcode, or default to NO2 if not detected
          ...
          char* comma5 = comma4 ? strchr(comma4 + 1, ',') : nullptr; // ADC_G position
          
          if (comma3 && comma4 && comma5) {
            // Extract Temperature (field 3: divide by 100 to get actual temperature)
            char temp_str[16];
              ...
              if (adc_g_len < sizeof(adc_g_str)) {
                ...
                if (comma7) {
                  ...
                  if (adc_t_len < sizeof(adc_t_str)) {
                    ...
                    while (*adc_t_start == ' ') adc_t_start++;
                    
                    adc_t_raw = (uint16_t)strtoul(adc_t_start, nullptr, 10);
                    data.rawT = adc_t_raw;
                    
                    // Extract ADC_H (raw humidity ADC value) - field 7 (last field)
                    char adc_h_str[16];
                    const char* adc_h_ptr = comma7 + 1;
                    size_t adc_h_len = strlen(adc_h_ptr);
                    // Remove trailing whitespace and newlines
                    while (adc_h_len > 0 && (adc_h_ptr[adc_h_len-1] == '\r' || adc_h_ptr[adc_h_len-1] == '\n' || adc_h_ptr[adc_h_len-1] == ' ')) {
                      adc_h_len--;
                    }
                  ...
                  // No comma7, so we have only 7 fields: Serial, PPB, Temp, Humi, ADC_G, ADC_T, ADC_H
                  // Extract ADC_T from comma5+1 to comma6
                  char adc_t_str[16];
          ...
          ESP_LOGI(TAG, "Gas concentration: %.1f PPB (%s)", ppb_value, data.gasType.c_str());
